...
public class VacancyDAO implements InterfaceDAO<Vacancy> {
        ...
        try (Connection connection = Service.getConnection()) {
            ...
            String READ_SQL =
                    "SELECT * FROM employment.vacancies " +
                            "LEFT JOIN employment.employers ON vacancies.employer_id = employers.id";
    ...
    @Override
    public Vacancy read(int id) throws MyException {
        Vacancy vacancy = null;
        try (Connection connection = Service.getConnection()) {
            String READ_SQL = "SELECT * FROM employment.vacancies, employment.employers " +
                    "WHERE vacancies.employer_id = employers.id AND vacancies.id = ?";
            ...
            if(resultSet.next()){
                ...
                vacancy.setDescription(resultSet.getString("vacancies.description"));
                vacancy.setSalary(resultSet.getInt("vacancies.salary"));
                vacancy.setPublicationDate(resultSet.getDate("vacancies.publication_date"));
            }

        } catch (SQLException e) {
            throw new MyException("class VacancyDAO method read(int id)", e);
        }
    ...
    public void update(Vacancy entity) throws MyException {
        ...
        try (Connection connection = Service.getConnection()) {
            ...
            throw new MyException("class VacancyDAO method update(Vacancy entity)", e);
        }
    }
